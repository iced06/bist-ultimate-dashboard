...
def display_balance_sheet(symbol, current_price=None):
    ...
    with tab_buffett:
        ...
        st.caption("Key metrics Warren Buffett looks for in company financials")
        ...
        if df_full is not None and not df_full.empty:
            ...
            operating_profit = bf_get("operating_profit")
            ...
            st.markdown(f"**Analysis Period:** {latest_p}")
            ...
            if gross_profit and revenue and revenue != 0:
                ...
                passed = gpm > 40
                buffett_checks.append(("Gross Profit Margin > 40%", f"{gpm:.1f}%", passed,
            ...
            if net_profit and revenue and revenue != 0:
                ...
                passed = npm > 20
                buffett_checks.append(("Net Profit Margin > 20%", f"{npm:.1f}%", passed,
                    "Strong net margins show pricing power and efficiency. >20% is excellent."))
            
            # 3. ROE > 15%
            roe = None
            if net_profit and total_equity and total_equity != 0:
                quarter = latest_p.split("/")[1] if latest_p else "12"
                ann = {"12": 1, "9": 12/9, "6": 12/6, "3": 12/3}.get(quarter, 1)
                roe = (net_profit * ann / total_equity) * 100
                passed = roe > 15
                buffett_checks.append(("ROE > 15%", f"{roe:.1f}%", passed,
                    "High ROE means the company generates strong returns on shareholder equity. Buffett target: >15%."))
            ...
            if net_profit and total_assets and total_assets != 0:
                quarter = latest_p.split("/")[1] if latest_p else "12"
                ann = {"12": 1, "9": 12/9, "6": 12/6, "3": 12/3}.get(quarter, 1)
                roa = (net_profit * ann / total_assets) * 100
                passed = roa > 7
                buffett_checks.append(("ROA > 7%", f"{roa:.1f}%", passed,
            ...
            if total_debt >= 0 and total_equity and total_equity > 0:
                ...
                passed = de < 0.5
                buffett_checks.append(("Debt/Equity < 0.5", f"{de:.2f}", passed,
                    "Buffett prefers companies with low debt. D/E < 0.5 means conservative financing."))
            ...
            if current_assets and current_liab and current_liab > 0:
                ...
                passed = cr > 1.5
                buffett_checks.append(("Current Ratio > 1.5", f"{cr:.2f}", passed,
                    "Adequate liquidity to meet short-term obligations. >1.5 is comfortable."))
            
            # 7. Earnings Consistency (check multiple periods for positive net profit)
            if period_cols_bf:
                np_row = df_full[df_full["LogicalKey"] == "net_profit_parent"] if "LogicalKey" in df_full.columns else pd.DataFrame()
                if np_row.empty:
                    np_row = df_full[df_full["LogicalKey"] == "net_profit"] if "LogicalKey" in df_full.columns else pd.DataFrame()
                if not np_row.empty:
                    pos_count = sum(1 for c in period_cols_bf if pd.to_numeric(np_row.iloc[0].get(c), errors='coerce') is not None and pd.to_numeric(np_row.iloc[0].get(c), errors='coerce') > 0)
                    total_periods = len(period_cols_bf)
                    pct = (pos_count / total_periods * 100) if total_periods > 0 else 0
                    passed = pct >= 80
                    buffett_checks.append(("Earnings Consistency ≥ 80%", f"{pos_count}/{total_periods} ({pct:.0f}%)", passed,
                        "Buffett wants consistent earners. ≥80% of periods with positive profit."))
            
            # 8. Revenue Growth (compare oldest to newest annual)
            annual_cols = [c for c in period_cols_bf if c.endswith("/12")]
            if len(annual_cols) >= 2:
                rev_row = df_full[df_full["LogicalKey"] == "revenue"] if "LogicalKey" in df_full.columns else pd.DataFrame()
                if not rev_row.empty:
                    latest_rev = pd.to_numeric(rev_row.iloc[0].get(annual_cols[0]), errors='coerce')
                    oldest_rev = pd.to_numeric(rev_row.iloc[0].get(annual_cols[-1]), errors='coerce')
                    if pd.notna(latest_rev) and pd.notna(oldest_rev) and oldest_rev > 0:
                        rev_growth = ((latest_rev - oldest_rev) / abs(oldest_rev)) * 100
                        passed = rev_growth > 0
                        buffett_checks.append(("Revenue Growth (Overall)", f"{rev_growth:+.1f}%", passed,
                            "Growing revenue is essential. Buffett wants companies that consistently grow top line."))
            
            # Display Buffett scorecard
            passed_count = sum(1 for _, _, p, _ in buffett_checks if p)
            ...
            # Individual checks
            for check_name, check_val, check_passed, check_desc in buffett_checks:
                icon = "✅" if check_passed else "❌"
                bg = "rgba(40,167,69,0.1)" if check_passed else "rgba(220,53,69,0.1)"
                border = "#28a745" if check_passed else "#dc3545"
                st.markdown(f"""
                <div style="border-left: 4px solid {border}; background: {bg}; padding: 0.8rem 1rem; margin: 0.4rem 0; border-radius: 0 6px 6px 0;">
                    <strong>{icon} {check_name}: {check_val}</strong>
                    <p style="margin: 0.3rem 0 0 0; font-size: 0.85rem; opacity: 0.85;">{check_desc}</p>
                </div>
                """, unsafe_allow_html=True)
            
            st.caption("Based on Warren Buffett's publicly known investment criteria. Not financial advice.")
        else:
...
def fetch_oecd_data(dataset, subject, country, measure, freq, start_year=2010):
    """Fetch from OECD stats SDMX-JSON API (free, no key)."""
    try:
        url = f"https://stats.oecd.org/sdmx-json/data/{dataset}/{subject}.{country}.{measure}.{freq}/all?startTime={start_year}"
        headers = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}
...
def fetch_cds_data(country_name):
    """Fetch 5Y CDS historical data by scraping WorldGovernmentBonds.com."""
    ...
    try:
        ...
        if resp.status_code == 200:
            # Parse HTML table
            import re as re_mod
            tables = re_mod.findall(r'<table[^>]*>(.*?)</table>', resp.text, re_mod.DOTALL)
            for table_html in tables:
                rows_html = re_mod.findall(r'<tr[^>]*>(.*?)</tr>', table_html, re_mod.DOTALL)
                rows = []
                for row_html in rows_html:
                    cells = re_mod.findall(r'<td[^>]*>(.*?)</td>', row_html, re_mod.DOTALL)
                    if len(cells) >= 2:
                        date_str = cells[0].strip().replace('\n', '').strip()
                        val_str = cells[1].strip().replace('\n', '').replace(',', '').strip()
                        try:
