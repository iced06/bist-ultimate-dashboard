...
def calculate_valuation_metrics(df_full, current_price, valuation_data=None):
    ...
    def get_ttm_value_tl(logical_key, fallback_key=None):
        ...
        latest_v = get_val(logical_key, latest)
        if latest_v is None and fallback_key:
            latest_v = get_val(fallback_key, latest)
        if latest_v is None:
            return None
    ...
    ebitda_direct = get_val("ebitda", latest)
...
COUNTRY_MACRO_CONFIG = {
    "🇺🇸 US": {
        "m2": {"tv": ["TVC:DXY"], "label": "US Dollar Index (DXY)", "unit": "Index"},
        "employment": {"tv": ["TVC:US10Y"], "label": "US 10Y Yield", "unit": "%"},
        "consumer_spending": {"tv": ["AMEX:SPY", "SP:SPX"], "label": "S&P 500", "unit": "Index"},
        "pce_index": {"tv": ["OANDA:XAUUSD", "COMEX:GC1!"], "label": "Gold Spot", "unit": "USD"},
        "inflation": {"tv": ["TVC:USOIL", "NYMEX:CL1!"], "label": "WTI Crude", "unit": "USD"},
        "pmi": {"tv": ["CBOE:VIX"], "label": "VIX Volatility Index", "unit": "Index"},
    },
    "🇩🇪 Germany": {
        "m2": {"tv": ["XETR:DAX", "TVC:DEU40"], "label": "DAX Index", "unit": "Index"},
        "employment": {"tv": ["TVC:DE10Y"], "label": "Germany 10Y Yield", "unit": "%"},
        "consumer_spending": {"tv": ["FX:EURUSD"], "label": "EUR/USD", "unit": "FX Rate"},
        "pce_index": {"tv": ["OANDA:XAUUSD", "COMEX:GC1!"], "label": "Gold Spot", "unit": "USD"},
        "inflation": {"tv": ["TVC:USOIL", "NYMEX:CL1!"], "label": "WTI Crude", "unit": "USD"},
        "pmi": {"tv": ["TVC:DXY"], "label": "Dollar Index (External Pressure)", "unit": "Index"},
    },
    "🇯🇵 Japan": {
        "m2": {"tv": ["TVC:NI225"], "label": "Nikkei 225", "unit": "Index"},
        "employment": {"tv": ["TVC:JP10Y"], "label": "Japan 10Y Yield", "unit": "%"},
        "consumer_spending": {"tv": ["FX:USDJPY"], "label": "USD/JPY", "unit": "FX Rate"},
        "pce_index": {"tv": ["OANDA:XAUUSD", "COMEX:GC1!"], "label": "Gold Spot", "unit": "USD"},
        "inflation": {"tv": ["TVC:USOIL", "NYMEX:CL1!"], "label": "WTI Crude", "unit": "USD"},
        "pmi": {"tv": ["TVC:DXY"], "label": "Dollar Index", "unit": "Index"},
    },
    "🇨🇳 China": {
        "m2": {"tv": ["SSE:000001", "TVC:SHCOMP"], "label": "Shanghai Composite", "unit": "Index"},
        "employment": {"tv": ["TVC:CN10Y"], "label": "China 10Y Yield", "unit": "%"},
        "consumer_spending": {"tv": ["FX:USDCNH"], "label": "USD/CNH", "unit": "FX Rate"},
        "pce_index": {"tv": ["OANDA:XAUUSD", "COMEX:GC1!"], "label": "Gold Spot", "unit": "USD"},
        "inflation": {"tv": ["TVC:USOIL", "NYMEX:CL1!"], "label": "WTI Crude", "unit": "USD"},
        "pmi": {"tv": ["HKEX:HSI"], "label": "Hang Seng Index", "unit": "Index"},
    },
    "🇮🇳 India": {
        "m2": {"tv": ["NSE:NIFTY"], "label": "NIFTY 50", "unit": "Index"},
        "employment": {"tv": ["TVC:IN10Y"], "label": "India 10Y Yield", "unit": "%"},
        "consumer_spending": {"tv": ["FX:USDINR"], "label": "USD/INR", "unit": "FX Rate"},
        "pce_index": {"tv": ["OANDA:XAUUSD", "COMEX:GC1!"], "label": "Gold Spot", "unit": "USD"},
        "inflation": {"tv": ["TVC:USOIL", "NYMEX:CL1!"], "label": "WTI Crude", "unit": "USD"},
        "pmi": {"tv": ["NSE:BANKNIFTY"], "label": "Bank Nifty", "unit": "Index"},
    },
    "🇹🇷 Turkey": {
        "m2": {"tv": ["BIST:XU100"], "label": "BIST 100", "unit": "Index"},
        "employment": {"tv": ["TVC:TR10Y"], "label": "Turkey 10Y Yield", "unit": "%"},
        "consumer_spending": {"tv": ["FX:USDTRY"], "label": "USD/TRY", "unit": "FX Rate"},
        "pce_index": {"tv": ["BIST:XBANK"], "label": "BIST Banks Index", "unit": "Index"},
        "inflation": {"tv": ["TVC:USOIL", "NYMEX:CL1!"], "label": "WTI Crude", "unit": "USD"},
        "pmi": {"tv": ["BIST:XUSIN"], "label": "BIST Industrials Index", "unit": "Index"},
    },
}

MACRO_ASSET_SYMBOLS = {
    "btc": {"tv": ["BINANCE:BTCUSDT", "BITSTAMP:BTCUSD"], "yahoo": "BTC-USD"},
    "gold": {"tv": ["OANDA:XAUUSD", "COMEX:GC1!"], "yahoo": "GC=F"},
    "silver": {"tv": ["OANDA:XAGUSD", "COMEX:SI1!"], "yahoo": "SI=F"},
}


@st.cache_data(ttl=21600, show_spinner=False)
def fetch_tradingview_series(symbols, start_date="2015-01-01", interval="1d"):
    """Fetch first available TradingView symbol via borsapy and return DataFrame(date,value)."""
    ...
    for sym in symbols:
        try:
            ...
            value_col = next((c for c in ["Close", "Adj Close"] if c in df.columns), None)
            if value_col is None:
                continue
            out = df[[value_col]].rename(columns={value_col: "value"}).dropna()
            out.index = pd.to_datetime(out.index)
            if interval == "1d":
                out = out.resample("W").last().dropna()
            return out
        except Exception:
            continue
    return None
...
def build_global_liquidity_proxy(start_date="2015-01-01"):
    ...
    """
    spx = fetch_tradingview_series(["AMEX:SPY", "SP:SPX"], start_date, interval="1d")
    dxy = fetch_tradingview_series(["TVC:DXY"], start_date, interval="1d")
    us10y = fetch_tradingview_series(["TVC:US10Y"], start_date, interval="1d")
...
def display_macro_analysis():
    ...
    st.caption("TradingView-based liquidity/risk dashboard and cross-market indicators")
    
    # ═══════════════════════════════════════════════════════════════════
    # TOP SECTION: Liquidity Proxy vs BTC, Gold, Silver
    # ═══════════════════════════════════════════════════════════════════
    
    with st.spinner("Loading TradingView macro and asset data..."):
        liquidity_df = build_global_liquidity_proxy("2015-01-01")
        btc_df = fetch_tradingview_series(MACRO_ASSET_SYMBOLS["btc"]["tv"], "2015-01-01", "1d")
        gold_df = fetch_tradingview_series(MACRO_ASSET_SYMBOLS["gold"]["tv"], "2015-01-01", "1d")
        silver_df = fetch_tradingview_series(MACRO_ASSET_SYMBOLS["silver"]["tv"], "2015-01-01", "1d")
        if btc_df is None:
            btc_df = fetch_asset_price(MACRO_ASSET_SYMBOLS["btc"]["yahoo"], "10y")
        if gold_df is None:
            gold_df = fetch_asset_price(MACRO_ASSET_SYMBOLS["gold"]["yahoo"], "10y")
        if silver_df is None:
            silver_df = fetch_asset_price(MACRO_ASSET_SYMBOLS["silver"]["yahoo"], "10y")
    ...
    if liquidity_df is not None:
        ...
        st.error("⚠️ Could not build TradingView liquidity proxy. Check symbol accessibility and connectivity.")
    ...
    with st.spinner(f"Loading {country} indicators from TradingView..."):
        indicator_data = {}
        for key, cfg in config.items():
            indicator_data[key] = fetch_tradingview_series(cfg["tv"], "2018-01-01", "1d")
    ...
    st.caption("Source: TradingView market symbols (with Yahoo fallback for precious metals/crypto when needed)")
