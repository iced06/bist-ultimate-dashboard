...
def calculate_valuation_metrics(df_full, current_price, valuation_data=None):
    ...
    if market_cap:
        ...
        if ttm_ebitda_tl and ttm_ebitda_tl != 0:
            result["ev_ebitda"] = round(ev / ttm_ebitda_tl, 2)
...
def calculate_forward_valuations(current_price, valuation, forecasts):
    ...
    Returns dict with forward_pe, forward_ev_ebitda and current pb (P/B doesn't change
    with earnings forecasts — it would need a forecast balance sheet).
    """
    result = {}
    UNIT = 1000
    ...
    # P/B stays current (balance sheet forecast would need more assumptions)
...
def display_balance_sheet(symbol, current_price=None):
    ...
    if df_full is not None:
        ...
        if has_forecasts:
            st.caption("🔮 Gold bars = AI forecast (4 quarters ahead) using seasonal + momentum + mean-reversion ensemble")
        
        # Get forecast dicts for each metric
        fc_rev = forecasts.get("revenue", None)
        fc_gp = forecasts.get("gross_profit", None)
        fc_ebitda = forecasts.get("ebitda", None)
        fc_np = forecasts.get("net_profit", None)
        
        # 2x2 grid of bar charts
        ch_c1, ch_c2 = st.columns(2)
        
        with ch_c1:
            fig_rev = _make_quarterly_bar_chart(df_full, "revenue", "📈 Revenue (Hasılat)", "#17a2b8", forecasts=fc_rev)
            if fig_rev:
        ...
        with ch_c2:
            fig_gp = _make_quarterly_bar_chart(df_full, "gross_profit", "📈 Gross Profit (Brüt Kar)", "#28a745", forecasts=fc_gp)
            if fig_gp:
        ...
        with ch_c3:
            fig_ebitda = _make_quarterly_bar_chart(df_full, "ebitda", "📈 EBITDA", "#fd7e14", forecasts=fc_ebitda)
            if fig_ebitda is None:
                fig_ebitda = _make_quarterly_bar_chart(df_full, "operating_profit", "📈 Operating Profit (EBIT)", "#fd7e14", forecasts=fc_ebitda)
            if fig_ebitda:
        ...
        with ch_c4:
            fig_np = _make_quarterly_bar_chart(df_full, "net_profit_parent", "📈 Net Profit (Net Kar)", "#6f42c1", fallback_key="net_profit", forecasts=fc_np)
            if fig_np:
        ...
        if has_forecasts and current_price and current_price > 0:
            ...
            if valuation:
                ...
                if fwd:
                    ...
                    # Current P/B (stays same)
                    with fvc2:
                        pb = fwd.get("current_pb")
                        if pb is not None:
                            pb_color = "#28a745" if pb < 1.5 else ("#ffc107" if pb < 3 else "#dc3545")
                            pb_label = "Below Book" if pb < 1 else ("Fair" if pb < 2 else ("Premium" if pb < 4 else "Very High"))
                            st.markdown(f"""
                            <div class="mobile-score-card" style="border-top: 3px solid gold; background: rgba(255,215,0,0.05);">
                                <p class="score-label">📊 P/B (Current)</p>
                                <p class="score-value" style="color:{pb_color}">{pb:.2f}x</p>
                                <p class="score-max">{pb_label}</p>
                            </div>
                            """, unsafe_allow_html=True)
                    ...
                    with st.expander("📋 Forecast Details"):
                        ...
                        st.caption("""
                        **Methodology:** Ensemble of 3 forecasting approaches:
                        - **Seasonal (50% weight):** Same-quarter-last-year values with dampened YoY growth rate
                        - **Momentum (30% weight):** Linear trend extrapolation from last 4 standalone quarters
                        - **Mean Reversion (20% weight):** Exponentially weighted average of historical same-quarter values
                        
                        Standalone quarters are de-cumulated from İş Yatırım's cumulative reporting format.
