...
<style>
    ...
    .stApp {
        background:
            ...
            radial-gradient(900px 500px at 120% 0%, rgba(0,168,150,0.18), transparent 55%),
            var(--bg-soft);
    }
    ...
    .hero-panel {
        background: linear-gradient(120deg, rgba(10,132,255,0.2) 0%, rgba(0,168,150,0.16) 100%), var(--surface);
        border: 1px solid rgba(167, 180, 200, 0.2);
    ...
    .kpi-card {
        ...
        box-shadow: 0 6px 14px rgba(16, 32, 53, 0.06);
    }
    .kpi-label {
        margin: 0;
        color: var(--text-muted);
    ...
    .sentiment-strong-bearish {
        ...
        font-weight: 800;
        margin: 0.9rem 0;
        box-shadow: 0 10px 20px rgba(16, 32, 53, 0.18);
    }
    .sentiment-strong-bullish { background: linear-gradient(135deg, #17b26a 0%, #13794b 100%); }
    .sentiment-bullish { background: linear-gradient(135deg, #1594ff 0%, #1366cc 100%); }
    .sentiment-neutral { background: linear-gradient(135deg, #f5b700 0%, #e67700 100%); }
    .sentiment-bearish { background: linear-gradient(135deg, #fd7e14 0%, #d6336c 100%); }
    .sentiment-strong-bearish { background: linear-gradient(135deg, #f03e3e 0%, #a5113a 100%); }
    ...
    .mobile-score-card {
        ...
        text-align: center;
        border: 1px solid rgba(167, 180, 200, 0.2);
        box-shadow: 0 8px 18px rgba(16, 32, 53, 0.06);
    }
    ...
    .stButton > button {
        border-radius: 10px;
        border: 1px solid rgba(167, 180, 200, 0.24);
        font-weight: 650;
        background: #1a2740;
        color: #e6edf7;
    }
...
def style_figure(fig, title=None, y_title=None, height=300):
    """Apply a unified visual style for all Plotly charts."""
    fig.update_layout(
        title=title,
        yaxis_title=y_title,
        height=height,
        ...
        font=dict(color="#dbe6f7"),
        margin=dict(l=36, r=16, t=48, b=60),
        legend=dict(
            orientation="h",
            yanchor="top",
            y=-0.16,
    ...
    fig.update_xaxes(
        showgrid=True,
        gridcolor="rgba(167, 180, 200, 0.18)",
        zeroline=False,
        title_standoff=8,
    )
    fig.update_yaxes(
...
def render_dashboard_header(mode, timeframe_key, auth_enabled):
    """Render a professional hero section with context KPIs."""
    auth_badge = "Real-time" if auth_enabled else "15-min Delay"
    fin_count = len(st.session_state.financial_store)
    market_cached = len(st.session_state.market_summary)
    st.markdown(
        f"""
        <div class="hero-panel">
            <h1 class="main-header">BIST Ultimate Analysis</h1>
            <p class="hero-subtitle">Professional technical and valuation dashboard for Turkish equities</p>
        </div>
        <div class="kpi-grid">
            <div class="kpi-card">
                <p class="kpi-label">Universe</p>
                <p class="kpi-value">{len(IMKB)} stocks</p>
            </div>
            <div class="kpi-card">
                <p class="kpi-label">Mode</p>
                <p class="kpi-value">{mode}</p>
            </div>
            <div class="kpi-card">
                <p class="kpi-label">Timeframe</p>
                <p class="kpi-value">{TIMEFRAMES[timeframe_key]["label"]}</p>
            </div>
            <div class="kpi-card">
                <p class="kpi-label">Data</p>
                <p class="kpi-value">{auth_badge} | Financials: {fin_count} | Cached Scans: {market_cached}</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def create_gauge(v, t, m=5):
    fig = go.Figure(go.Indicator(
        ...
        gauge={'axis': {'range': [None, m]}, 'bar': {'color': "#0a84ff"},
               'steps': [{'range': [0, 2], 'color': "#ffe3e3"},
                        {'range': [2, 3.5], 'color': "#fff3bf"},
                        {'range': [3.5, m], 'color': "#d3f9d8"}]}
    ))
    fig.update_layout(
        height=180,
...
def create_candlestick(df, s):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['bb_bbh'], mode='lines', line=dict(color='rgba(240,62,62,0.65)', width=1), name='BB Upper'))
    fig.add_trace(go.Scatter(x=df.index, y=df['bb_bbl'], mode='lines', line=dict(color='rgba(10,132,255,0.65)', width=1), name='BB Lower', fill='tonexty', fillcolor='rgba(10,132,255,0.08)'))
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA5'], mode='lines', line=dict(color='#f59f00', width=2), name='SMA 5'))
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA22'], mode='lines', line=dict(color='#1098ad', width=2), name='SMA 22'))
    style_figure(fig, title=f"{s} Price Action", height=390)
    fig.update_layout(xaxis_rangeslider_visible=False)
    return fig

def create_volume_chart(df):
    fig = go.Figure()
    colors = ['#fa5252' if df['Close'].iloc[i] < df['Open'].iloc[i] else '#2f9e44' for i in range(len(df))]
    fig.add_trace(go.Bar(x=df.index, y=df['Volume'], name='Volume', marker_color=colors))
    fig.add_trace(go.Scatter(x=df.index, y=df['VSMA15'], mode='lines', line=dict(color='#0a84ff', width=2), name='Volume SMA 15'))
    style_figure(fig, title="Volume Analysis", y_title="Volume", height=280)
    return fig

def create_macd_chart(df):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df['MACD'], mode='lines', line=dict(color='#0a84ff', width=2), name='MACD'))
    fig.add_trace(go.Scatter(x=df.index, y=df['MACDS'], mode='lines', line=dict(color='#f03e3e', width=2), name='Signal'))
    colors = ['#2f9e44' if val > 0 else '#fa5252' for val in df['Diff']]
    fig.add_trace(go.Bar(x=df.index, y=df['Diff'], name='Histogram', marker_color=colors))
    style_figure(fig, title="MACD Indicator", y_title="Value", height=280)
    return fig

def create_rsi_chart(df):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], mode='lines', line=dict(color='#7048e8', width=2), name='RSI'))
    fig.add_hline(y=70, line_dash="dash", line_color="red", annotation_text="Overbought (70)")
    fig.add_hline(y=30, line_dash="dash", line_color="green", annotation_text="Oversold (30)")
    style_figure(fig, title="RSI Indicator", y_title="RSI", height=280)
...
KEY_INCOME_BY_DESC = {
    ...
    "ebitda":              ("EBITDA",                    ["favök", "fvaök", "favok", "fvaok", "ebitda"]),
    "depreciation":        ("Depreciation & Amort.",     ["amortisman ve itfa gideri", "amortisman gideri", "amortisman"]),
    "operating_profit":    ("Operating Profit / EBIT",   ["esas faaliyet karı", "esas faaliyet kârı"]),
...
def calculate_valuation_metrics(df_full, current_price, valuation_data=None):
    ...
    def get_ttm_value_tl(logical_key, fallback_key=None):
        """
        Get TTM value in TL from cumulative periods.
        Priority:
        1. Latest annual /12
        2. Annualized latest interim period
        """
        annual_periods = [p for p in periods if p.endswith("/12")]
        for p in annual_periods:
            v = get_val(logical_key, p)
            if v is None and fallback_key:
                v = get_val(fallback_key, p)
            if v is not None:
                return v * UNIT

        latest_period_for_key = None
        latest_v = None
        for p in periods:
            v = get_val(logical_key, p)
            if v is None and fallback_key:
            ...
            if v is not None:
                latest_v = v
                latest_period_for_key = p
                break
        if latest_v is None or latest_period_for_key is None:
            return None
        quarter = latest_period_for_key.split("/")[1]
        annualization_factor = {"12": 1, "9": 12/9, "6": 12/6, "3": 12/3}.get(quarter, 1)
        return latest_v * UNIT * annualization_factor

    def get_latest_val(logical_key, fallback_key=None):
        """Get latest non-null value for a logical key and its period."""
        for p in periods:
            v = get_val(logical_key, p)
            if v is None and fallback_key:
                v = get_val(fallback_key, p)
    ...
    latest = periods[0]  # Display fallback period
    
    # All values below are in THOUSANDS TL as returned by the API
    total_equity, total_equity_period = get_latest_val("total_equity")
    paid_in_capital, _ = get_latest_val("paid_in_capital")
    net_profit, net_profit_period = get_latest_val("net_profit_parent", fallback_key="net_profit")
    ebitda_direct, ebitda_period = get_latest_val("ebitda")
    operating_profit, operating_profit_period = get_latest_val("operating_profit")
    depreciation, _ = get_latest_val("depreciation")
    short_debt, _ = get_latest_val("short_borrowings")
    long_debt, _ = get_latest_val("long_borrowings")
    cash, _ = get_latest_val("cash")

    period_label = next((p for p in [ebitda_period, operating_profit_period, net_profit_period, total_equity_period, latest] if p), latest)
    ...
    ttm_net_profit_tl = get_ttm_value_tl("net_profit_parent", fallback_key="net_profit")
    ...
    if ebitda_direct is not None:
        ebitda_thousands = ebitda_direct
    ...
    if ebitda_thousands is not None:
        result["ebitda_period"] = ebitda_thousands
    
    # ── TTM EBITDA (annualize if interim period) ──
    ttm_ebitda_tl = get_ttm_value_tl("ebitda")
    if ttm_ebitda_tl is None and ebitda_thousands is not None and ebitda_thousands > 0:
        quarter = (ebitda_period or operating_profit_period or latest).split("/")[1]
        annualization_factor = {"12": 1, "9": 12/9, "6": 12/6, "3": 12/3}.get(quarter, 1)
    ...
    if market_cap:
        ...
        if ttm_ebitda_tl and ttm_ebitda_tl > 0:
            ...
            # Sanity: EV/EBITDA typically 3-100x for real companies
            if 0.5 < ev_ebitda < 200:
                result["ev_ebitda"] = round(ev_ebitda, 2)
                result["ttm_ebitda"] = ttm_ebitda_tl

        # Fallback to market-provided EV/EBITDA if financial statement EBITDA is unavailable
        if "ev_ebitda" not in result and valuation_data and isinstance(valuation_data, dict):
            for v in valuation_data.values():
                ...
                if raw_val is None:
                    continue
                if ("fav" in desc and "fd" in desc) or ("ebitda" in desc and ("ev" in desc or "firma" in desc)):
                    try:
                        ev_ratio = float(raw_val)
                        if 0.5 < ev_ratio < 200:
                            result["ev_ebitda"] = round(ev_ratio, 2)
                            break
                    except Exception:
                        continue
    ...
    if total_equity and total_equity > 0 and net_profit and net_profit != 0:
        quarter = (net_profit_period or period_label).split("/")[1]
...
def _match_item_by_desc(item_desc_tr, keyword_patterns):
    ...
    def _normalize_tr_text(s):
        if s is None:
            return ""
        s = str(s).lower().strip()
        # Handle dotted-i composed form before translate (multi-char sequence)
        s = s.replace("i̇", "i")
    ...
    desc_lower = _normalize_tr_text(item_desc_tr)
    for pattern in keyword_patterns:
        if _normalize_tr_text(pattern) in desc_lower:
            return True
    return False


def _find_fin_row(df_source, logical_keys=None, keyword_patterns=None):
    """Find best-matching financial row by logical key first, then Turkish description keywords."""
    if df_source is None or df_source.empty:
        return pd.DataFrame()

    logical_keys = logical_keys or []
    keyword_patterns = keyword_patterns or []

    if "LogicalKey" in df_source.columns:
        for lk in logical_keys:
            row = df_source[df_source["LogicalKey"] == lk]
            if not row.empty:
                return row

    if "Item (TR)" in df_source.columns and keyword_patterns:
        for p in keyword_patterns:
            mask = df_source["Item (TR)"].fillna("").apply(lambda x: _match_item_by_desc(x, [p]))
            row = df_source[mask]
            if not row.empty:
                return row
...
def _append_ebitda_row_if_missing(df_source):
    """Append calculated EBITDA row if an explicit EBITDA row is missing."""
    if df_source is None or df_source.empty:
        return df_source

    if "LogicalKey" in df_source.columns:
        existing = df_source[df_source["LogicalKey"] == "ebitda"]
        if not existing.empty:
            period_cols = [c for c in df_source.columns if "/" in c]
            has_vals = any(pd.notna(pd.to_numeric(existing.iloc[0].get(c), errors="coerce")) for c in period_cols)
            if has_vals:
                return df_source
    ...
    op_row = _find_fin_row(
        df_source,
        logical_keys=["operating_profit"],
        keyword_patterns=["esas faaliyet kari", "faaliyet kari", "operating profit", "ebit"],
    )
    da_row = _find_fin_row(
        df_source,
        logical_keys=["depreciation"],
        keyword_patterns=["amortisman", "itfa"],
    )
    ...
    if not has_any:
        return df_source
    return pd.concat([df_source, pd.DataFrame([ebitda_row_data])], ignore_index=True)
...
def compute_stock_valuations(symbol, current_price):
    ...
    try:
        ...
        # Add calculated EBITDA row when source data does not provide one
        df_full = _append_ebitda_row_if_missing(df_full)
        
        # Current valuations
        market_valuation_data = fetch_valuation_data(symbol)
        valuation = calculate_valuation_metrics(df_full, current_price, valuation_data=market_valuation_data)
        if valuation:
        ...
        if result.get("pe") is not None and result.get("fwd_pe") is not None:
            result["pe_delta"] = round(result["pe"] - result["fwd_pe"], 2)
        if result.get("pb") is not None and result.get("fwd_pb") is not None:
            result["pb_delta"] = round(result["pb"] - result["fwd_pb"], 2)
        if result.get("ev_ebitda") is not None and result.get("fwd_ev_ebitda") is not None:
...
def display_balance_sheet(symbol, current_price=None):
    ...
    if df_full is not None:
        ...
        # Ensure EBITDA exists before charting (direct row or calculated fallback)
        df_full = _append_ebitda_row_if_missing(df_full)
...
# =============================================================================
# MACRO ANALYSIS — TradingView-based Macro Dashboard
# =============================================================================

COUNTRY_MACRO_CONFIG = {
    "🇺🇸 US": {
        "meta": {"wb_country": "USA", "te_country": "united states"},
        "m2": {"wb": "FM.LBL.BMNY.CN", "label": "M2 Money Supply", "unit": "Local Currency"},
        "employment": {"wb": "SL.EMP.TOTL.SP.ZS", "label": "Employment", "unit": "% of Population 15+"},
        "consumer_spending": {"wb": "NE.CON.PRVT.CN", "label": "Consumer Spending", "unit": "Local Currency"},
        "pce_index": {"wb": "NE.CON.PRVT.KD.ZG", "label": "Personal Consumption Expenditures", "unit": "% Growth"},
        "inflation": {"wb": "FP.CPI.TOTL.ZG", "label": "Inflation", "unit": "%"},
        "pmi": {"te_indicator": "manufacturing pmi", "label": "Manufacturing PMI", "unit": "Index"},
    },
    "🇩🇪 Germany": {
        "meta": {"wb_country": "DEU", "te_country": "germany"},
        "m2": {"wb": "FM.LBL.BMNY.CN", "label": "M2 Money Supply", "unit": "Local Currency"},
        "employment": {"wb": "SL.EMP.TOTL.SP.ZS", "label": "Employment", "unit": "% of Population 15+"},
        "consumer_spending": {"wb": "NE.CON.PRVT.CN", "label": "Consumer Spending", "unit": "Local Currency"},
        "pce_index": {"wb": "NE.CON.PRVT.KD.ZG", "label": "Personal Consumption Expenditures", "unit": "% Growth"},
    ...
    "🇯🇵 Japan": {
        "meta": {"wb_country": "JPN", "te_country": "japan"},
        "m2": {"wb": "FM.LBL.BMNY.CN", "label": "M2 Money Supply", "unit": "Local Currency"},
        "employment": {"wb": "SL.EMP.TOTL.SP.ZS", "label": "Employment", "unit": "% of Population 15+"},
        "consumer_spending": {"wb": "NE.CON.PRVT.CN", "label": "Consumer Spending", "unit": "Local Currency"},
        "pce_index": {"wb": "NE.CON.PRVT.KD.ZG", "label": "Personal Consumption Expenditures", "unit": "% Growth"},
    ...
    "🇨🇳 China": {
        "meta": {"wb_country": "CHN", "te_country": "china"},
        "m2": {"wb": "FM.LBL.BMNY.CN", "label": "M2 Money Supply", "unit": "Local Currency"},
        "employment": {"wb": "SL.EMP.TOTL.SP.ZS", "label": "Employment", "unit": "% of Population 15+"},
        "consumer_spending": {"wb": "NE.CON.PRVT.CN", "label": "Consumer Spending", "unit": "Local Currency"},
        "pce_index": {"wb": "NE.CON.PRVT.KD.ZG", "label": "Personal Consumption Expenditures", "unit": "% Growth"},
    ...
    "🇮🇳 India": {
        "meta": {"wb_country": "IND", "te_country": "india"},
        "m2": {"wb": "FM.LBL.BMNY.CN", "label": "M2 Money Supply", "unit": "Local Currency"},
        "employment": {"wb": "SL.EMP.TOTL.SP.ZS", "label": "Employment", "unit": "% of Population 15+"},
        "consumer_spending": {"wb": "NE.CON.PRVT.CN", "label": "Consumer Spending", "unit": "Local Currency"},
        "pce_index": {"wb": "NE.CON.PRVT.KD.ZG", "label": "Personal Consumption Expenditures", "unit": "% Growth"},
    ...
    "🇹🇷 Turkey": {
        "meta": {"wb_country": "TUR", "te_country": "turkey"},
        "m2": {"wb": "FM.LBL.BMNY.CN", "label": "M2 Money Supply", "unit": "Local Currency"},
        "employment": {"wb": "SL.EMP.TOTL.SP.ZS", "label": "Employment", "unit": "% of Population 15+"},
        "consumer_spending": {"wb": "NE.CON.PRVT.CN", "label": "Consumer Spending", "unit": "Local Currency"},
        "pce_index": {"wb": "NE.CON.PRVT.KD.ZG", "label": "Personal Consumption Expenditures", "unit": "% Growth"},
...
MACRO_ASSET_SYMBOLS = {
    "btc": {"symbols": ["BTC-USD", "IBIT"]},
    "gold": {"symbols": ["GC=F", "GLD"]},
    "silver": {"symbols": ["SI=F", "SLV"]},
}


@st.cache_data(ttl=86400, show_spinner=False)
def fetch_asset_price(ticker, period="10y"):
    """Fetch asset price using yfinance-style approach via Yahoo Finance."""
    try:
        end = datetime.now()
...
def fetch_market_series(symbols, start_date="2015-01-01"):
    """Fetch first available Yahoo symbol and return weekly DataFrame(date,value)."""
    if isinstance(symbols, str):
        symbols = [symbols]
    start_ts = pd.to_datetime(start_date)
    for sym in symbols:
        df = fetch_asset_price(sym, "10y")
        if df is None or df.empty:
            continue
        out = df.copy()
        col = "close" if "close" in out.columns else None
        if col is None:
            continue
        out = out[[col]].rename(columns={col: "value"}).dropna()
...
@st.cache_data(ttl=86400, show_spinner=False)
def fetch_world_bank_series(country_code, indicator_code, start_year=2000):
    """Fetch annual macro indicator from World Bank API."""
    ...
    try:
        ...
        for r in records:
            ...
            if val is None or yr is None:
                continue
            try:
                dt = pd.to_datetime(f"{int(yr)}-12-31")
...
def fetch_tradingeconomics_indicator(country_name, indicator_name):
    """Fetch indicator series from TradingEconomics guest API."""
    try:
        cname = country_name.replace(" ", "%20")
        iname = indicator_name.replace(" ", "%20")
        url = f"https://api.tradingeconomics.com/historical/country/{cname}/indicator/{iname}"
        ...
        for r in data:
            ...
            if val is None or dt is None:
                continue
            try:
                rows.append({"date": pd.to_datetime(dt), "value": float(val)})
            except Exception:
                continue
        if not rows:
            return None
...
def build_global_liquidity_proxy(start_date="2015-01-01"):
    """
    Build TradingView-based liquidity proxy:
    S&P500 (risk appetite) + inverse DXY + inverse US10Y (financial conditions).
    """
    components = []

    spx = fetch_market_series(["^GSPC", "SPY"], start_date)
    if spx is not None and not spx.empty:
        s = (spx["value"] / spx["value"].iloc[0]) * 100
        components.append(s.rename("spx"))

    dxy = fetch_market_series(["DX-Y.NYB", "UUP"], start_date)
    if dxy is not None and not dxy.empty:
        s = (dxy["value"] / dxy["value"].iloc[0]) * 100
        components.append((200 - s).rename("dxy_inv"))

    us10y = fetch_market_series(["^TNX", "IEF"], start_date)
    if us10y is not None and not us10y.empty:
        s = (us10y["value"] / us10y["value"].iloc[0]) * 100
        components.append((200 - s).rename("us10y_inv"))

    if not components:
        return None

    merged = pd.concat(components, axis=1).sort_index().ffill().dropna()
    if merged.empty:
        return None

    liquidity = merged.mean(axis=1)
    return pd.DataFrame({"liquidity_proxy": liquidity}, index=merged.index)


def create_liquidity_vs_asset_chart(liquidity_df, asset_df, asset_name, asset_color, lag_weeks=0):
    """Create a dual-axis chart: TradingView liquidity proxy vs an asset with optional lag."""
    if liquidity_df is None or asset_df is None:
        return None
    fig = go.Figure()

    proxy_plot = liquidity_df.copy()
    if lag_weeks > 0:
        proxy_plot.index = proxy_plot.index - pd.Timedelta(weeks=lag_weeks)

    asset_norm = asset_df.copy()
    asset_col = "close" if "close" in asset_norm.columns else "value"
    base = asset_norm[asset_col].iloc[0]
    asset_norm["normalized"] = (asset_norm[asset_col] / base) * 100

    common_start = max(proxy_plot.index.min(), asset_norm.index.min())
    common_end = min(proxy_plot.index.max(), asset_norm.index.max())
    proxy_plot = proxy_plot[common_start:common_end]
    asset_norm = asset_norm[common_start:common_end]

    fig.add_trace(go.Scatter(
        x=proxy_plot.index, y=proxy_plot["liquidity_proxy"],
        mode='lines', name=f"Liquidity Proxy{f' (shifted {lag_weeks}w)' if lag_weeks else ''}",
    ...
    lag_text = f" (Proxy shifted {lag_weeks}w forward)" if lag_weeks else ""
    fig.update_layout(
        title=f"Liquidity Proxy vs {asset_name}{lag_text}",
        height=350,
        plot_bgcolor="#0f1726",
        paper_bgcolor="#0f1726",
        font=dict(color="#dbe6f7"),
        ...
        xaxis=dict(title=""),
        yaxis=dict(title="Liquidity Proxy (Indexed)", side="left", showgrid=False),
...
def display_macro_analysis():
    ...
    st.caption("Liquidity proxy (market-based) + macro indicators by country")
    
    # ═══════════════════════════════════════════════════════════════════
    # TOP SECTION: Liquidity Proxy vs BTC, Gold, Silver
    # ═══════════════════════════════════════════════════════════════════
    
    with st.spinner("Loading macro and asset data..."):
        liquidity_df = build_global_liquidity_proxy("2015-01-01")
        btc_df = fetch_market_series(MACRO_ASSET_SYMBOLS["btc"]["symbols"], "2015-01-01")
        gold_df = fetch_market_series(MACRO_ASSET_SYMBOLS["gold"]["symbols"], "2015-01-01")
        silver_df = fetch_market_series(MACRO_ASSET_SYMBOLS["silver"]["symbols"], "2015-01-01")

    if liquidity_df is not None:
        st.markdown("#### 💰 Liquidity Proxy vs Risk Assets")
        st.caption("Proxy combines S&P500 strength, inverse DXY, and inverse US10Y yield. This usually reflects global risk-on liquidity conditions.")

        fig_btc = create_liquidity_vs_asset_chart(liquidity_df, btc_df, "Bitcoin (BTC)", "#f7931a", lag_weeks=6)
        if fig_btc:
        ...
        with gc1:
            fig_gold = create_liquidity_vs_asset_chart(liquidity_df, gold_df, "Gold", "#ffd700", lag_weeks=0)
            if fig_gold:
                st.plotly_chart(fig_gold, use_container_width=True, config=PLOTLY_CONFIG)
            else:
                st.warning("Could not load Gold data")
        with gc2:
            fig_silver = create_liquidity_vs_asset_chart(liquidity_df, silver_df, "Silver", "#c0c0c0", lag_weeks=0)
            if fig_silver:
                st.plotly_chart(fig_silver, use_container_width=True, config=PLOTLY_CONFIG)
            else:
                st.warning("Could not load Silver data")
    else:
        st.error("⚠️ Could not build liquidity proxy. Check connectivity or Yahoo symbol access.")
    
    # ═══════════════════════════════════════════════════════════════════
    # BOTTOM SECTION: Country-specific Economic Indicators
    # ═══════════════════════════════════════════════════════════════════
    st.markdown("---")
    st.markdown("### 🏛️ Country Economic Indicators")
    
    country = st.selectbox(
        "Select Country",
        list(COUNTRY_MACRO_CONFIG.keys()),
        index=0,
        key="macro_country"
    )
    ...
    wb_country = meta.get("wb_country")
    te_country = meta.get("te_country")
    
    indicator_colors = {
        "m2": "#00bfff",
        "employment": "#28a745",
        "consumer_spending": "#fd7e14",
        "pce_index": "#dc3545",
        "inflation": "#ff6b6b",
        "pmi": "#6f42c1",
    }
    ...
    indicator_labels = {
        ...
        "pce_index": "Personal Consumption Expenditures",
        "inflation": "Inflation",
    ...
    with st.spinner(f"Loading {country} indicators..."):
        indicator_data = {}
        for key in ["m2", "employment", "consumer_spending", "pce_index", "inflation", "pmi"]:
            cfg = config.get(key, {})
            if key == "pmi":
                pmi_df = fetch_tradingeconomics_indicator(te_country, cfg.get("te_indicator", "manufacturing pmi")) if te_country else None
                if (pmi_df is None or pmi_df.empty) and wb_country:
                    # Fallback proxy if PMI endpoint is unavailable
                    pmi_df = fetch_world_bank_series(wb_country, "NV.IND.MANF.KD.ZG", start_year=2000)
                indicator_data[key] = pmi_df
            else:
                wb_code = cfg.get("wb")
                indicator_data[key] = fetch_world_bank_series(wb_country, wb_code, start_year=2000) if wb_country and wb_code else None
    
    # Display in 2-column grid (3 rows x 2 cols = 6 indicators)
    ...
    for row_idx in range(0, len(keys), 2):
        ...
        for ci, col in enumerate([col1, col2]):
            ...
            with col:
                if df is not None and not df.empty:
                    fig = create_country_indicator_chart(
                        df, cfg["label"], cfg["unit"], indicator_colors.get(key, "#17a2b8")
                    )
                    if fig:
                        st.plotly_chart(fig, use_container_width=True, config=PLOTLY_CONFIG)
                        # Latest value
                    ...
                    st.info(f"{indicator_labels[key]}: Data not available")
    
    st.caption("Sources: World Bank Open Data (M2, Employment, Consumer Spending, PCE proxy, Inflation) + TradingEconomics guest API (Manufacturing PMI, with fallback proxy)")
...
def main():
    try:
        ...
        with st.sidebar:
            ...
            st.caption("Pick a workflow, then run scans/imports from this panel.")
            st.markdown("---")
        ...
        render_dashboard_header(mode, selected_tf, auth)
            ...
            if selected_tf in st.session_state.chosen_stocks and st.session_state.chosen_stocks[selected_tf]:
                ...
                for idx, s in enumerate(df_c.to_dict('records')):
                    with cols[idx % len(cols)]:
                        pe_str = f"{s['P/E']:.1f}x" if pd.notna(s.get('P/E')) else "—"
                        pb_str = f"{s['PD/DD']:.2f}x" if pd.notna(s.get('PD/DD')) else "—"
                        ev_str = f"{s['EV/EBITDA']:.1f}x" if pd.notna(s.get('EV/EBITDA')) else "—"
                        fpe_str = f"{s['Fwd P/E']:.1f}x" if pd.notna(s.get('Fwd P/E')) else "—"
                        fev_str = f"{s['Fwd EV/EBITDA']:.1f}x" if pd.notna(s.get('Fwd EV/EBITDA')) else "—"
                        st.markdown(f"""
