...
<style>
    :root {
        ...
        --radius-lg: 16px;
        --radius-md: 12px;
        --shadow-soft: 0 12px 28px rgba(16, 32, 53, 0.08);
    }

    .stApp {
        background:
            radial-gradient(1200px 600px at -20% -10%, rgba(10,132,255,0.12), transparent 60%),
            radial-gradient(900px 500px at 120% 0%, rgba(0,168,150,0.1), transparent 55%),
            var(--bg-soft);
    }
    ...
    .hero-panel {
        background: linear-gradient(120deg, rgba(10,132,255,0.12) 0%, rgba(0,168,150,0.1) 100%), var(--surface);
    ...
    .mobile-score-card {
        ...
        padding: 0.72rem;
        text-align: center;
        border: 1px solid rgba(16, 32, 53, 0.1);
    ...
    [data-testid="stMetric"] {
        ...
        border-radius: var(--radius-md);
        padding: 0.7rem 0.8rem;
        box-shadow: 0 5px 12px rgba(16, 32, 53, 0.05);
    }
...
def calculate_valuation_metrics(df_full, current_price, valuation_data=None):
    ...
    # If EBITDA seems unreasonably small relative to revenue, the operating_profit
    # may have matched the wrong line. Try using gross_profit as sanity check.
    gross_profit = get_val("gross_profit", latest)
    if (ttm_ebitda_tl and gross_profit and gross_profit > 0 and
        ebitda_thousands < gross_profit * 0.01):
        # EBITDA is less than 1% of gross profit — likely a matching error
        # Fall back: skip this EBITDA
        ttm_ebitda_tl = None
        ebitda_thousands = None
...
FRED_API_KEY = "DEMO_KEY"  # Public demo key — limited but functional

COUNTRY_MACRO_CONFIG = {
    "🇺🇸 US": {
        "m2": {"fred": "WM2NS", "label": "US M2 Money Supply", "unit": "Billion $"},
        "employment": {"fred": "PAYEMS", "label": "US Nonfarm Payrolls", "unit": "Thousands"},
        "consumer_spending": {"fred": "PCE", "label": "US Personal Consumption Expenditures", "unit": "Billion $"},
        "pce_index": {"fred": "PCEPI", "label": "US PCE Price Index", "unit": "Index 2017=100"},
        "inflation": {"fred": "CPIAUCSL", "label": "US CPI (All Urban)", "unit": "Index 1982-84=100"},
        "pmi": {"fred": "MANEMP", "label": "US Manufacturing Employment", "unit": "Thousands"},
    },
    "🇩🇪 Germany": {
        "m2": {"fred": "MANMM101DEM189S", "label": "Germany M2 Money Supply", "unit": "Index"},
        "employment": {"fred": "LREM64TTDEM156S", "label": "Germany Employment Rate", "unit": "%"},
        "consumer_spending": {"fred": "NAEXKP02DEQ189S", "label": "Germany Private Consumption", "unit": "Index"},
        "pce_index": {"fred": "DEUCPIALLMINMEI", "label": "Germany CPI", "unit": "Index 2015=100"},
        "inflation": {"fred": "FPCPITOTLZGDEU", "label": "Germany Inflation Rate", "unit": "% YoY"},
        "pmi": {"fred": "BSCICP03DEM665S", "label": "Germany Business Confidence", "unit": "Index"},
    },
    "🇯🇵 Japan": {
        "m2": {"fred": "MANMM101JPM189S", "label": "Japan M2 Money Supply", "unit": "Index"},
        "employment": {"fred": "LREM64TTJPM156S", "label": "Japan Employment Rate", "unit": "%"},
        "consumer_spending": {"fred": "NAEXKP02JPQ189S", "label": "Japan Private Consumption", "unit": "Index"},
        "pce_index": {"fred": "JPNCPIALLMINMEI", "label": "Japan CPI", "unit": "Index 2015=100"},
        "inflation": {"fred": "FPCPITOTLZGJPN", "label": "Japan Inflation Rate", "unit": "% YoY"},
        "pmi": {"fred": "BSCICP03JPM665S", "label": "Japan Business Confidence", "unit": "Index"},
    },
    "🇨🇳 China": {
        "m2": {"fred": "MANMM101CNM189S", "label": "China M2 Money Supply", "unit": "Index"},
        "employment": {"fred": "LREM64TTCNM156S", "label": "China Employment Rate", "unit": "%"},
        "consumer_spending": {"fred": "NAEXKP02CNQ189S", "label": "China Private Consumption", "unit": "Index"},
        "pce_index": {"fred": "CHNCPIALLMINMEI", "label": "China CPI", "unit": "Index 2015=100"},
        "inflation": {"fred": "FPCPITOTLZGCHN", "label": "China Inflation Rate", "unit": "% YoY"},
        "pmi": {"fred": "BSCICP03CNM665S", "label": "China Business Confidence", "unit": "Index"},
    },
    "🇮🇳 India": {
        "m2": {"fred": "MANMM101INM189S", "label": "India M2 Money Supply", "unit": "Index"},
        "employment": {"fred": "LREM64TTINM156S", "label": "India Employment Rate", "unit": "%"},
        "consumer_spending": {"fred": "NAEXKP02INQ189S", "label": "India Private Consumption", "unit": "Index"},
        "pce_index": {"fred": "INDCPIALLMINMEI", "label": "India CPI", "unit": "Index 2015=100"},
        "inflation": {"fred": "FPCPITOTLZGIND", "label": "India Inflation Rate", "unit": "% YoY"},
        "pmi": {"fred": "BSCICP03INM665S", "label": "India Business Confidence", "unit": "Index"},
    },
    "🇹🇷 Turkey": {
        "m2": {"fred": "MANMM101TRM189S", "label": "Turkey M2 Money Supply", "unit": "Index"},
        "employment": {"fred": "LREM64TTTRM156S", "label": "Turkey Employment Rate", "unit": "%"},
        "consumer_spending": {"fred": "NAEXKP02TRQ189S", "label": "Turkey Private Consumption", "unit": "Index"},
        "pce_index": {"fred": "TURCPIALLMINMEI", "label": "Turkey CPI", "unit": "Index 2015=100"},
        "inflation": {"fred": "FPCPITOTLZGTUR", "label": "Turkey Inflation Rate", "unit": "% YoY"},
        "pmi": {"fred": "BSCICP03TRM665S", "label": "Turkey Business Confidence", "unit": "Index"},
    },
}

# Global M2 = sum of major economies' M2 (approximated by US + EU + Japan + China)
GLOBAL_M2_SERIES = {
    "US_M2": "WM2NS",           # US M2 in Billions $
    "EU_M2": "MYAGM2EZM196N",   # Euro Area M2 in Millions €
    "JP_M2": "MABMM201JPM189S", # Japan M2 broad
    "CN_M2": "MABMM201CNM189S", # China M2 broad
}


@st.cache_data(ttl=86400, show_spinner=False)
def fetch_fred_series(series_id, start_date="2015-01-01"):
    """Fetch a FRED data series. Returns DataFrame with date index and value column."""
    try:
        params = {
            "series_id": series_id,
...
def build_global_m2_index(start_date="2015-01-01"):
    """
    Build a normalized global M2 index by fetching US M2 (dominant component)
    and normalizing to base 100. Returns weekly-resampled DataFrame.
    """
    # US M2 is the most reliable and dominant component
    us_m2 = fetch_fred_series("WM2NS", start_date)
    if us_m2 is None or us_m2.empty:
        return None
    
    # Normalize to base 100 at the start
    base = us_m2["value"].iloc[0]
    us_m2["global_m2"] = (us_m2["value"] / base) * 100
    
    # Resample to weekly for alignment with asset prices
    weekly = us_m2[["global_m2"]].resample("W").last().ffill()
    return weekly
...
def create_m2_vs_asset_chart(m2_df, asset_df, asset_name, asset_color, lag_weeks=0):
    ...
    # M2 line (potentially lagged forward to show leading)
    m2_plot = m2_df.copy()
    if lag_weeks > 0:
        m2_plot.index = m2_plot.index - pd.Timedelta(weeks=lag_weeks)
    
    # Normalize asset to base 100
    asset_norm = asset_df.copy()
    base = asset_norm["close"].iloc[0]
    asset_norm["normalized"] = (asset_norm["close"] / base) * 100
    ...
    lag_text = f" (M2 shifted {lag_weeks}w forward)" if lag_weeks else ""
    fig.update_layout(
...
def display_macro_analysis():
    ...
    # ═══════════════════════════════════════════════════════════════════
    # TOP SECTION: Global M2 vs BTC, Gold, Silver
    # ═══════════════════════════════════════════════════════════════════
    
    with st.spinner("Loading global M2 and asset data..."):
        m2_df = build_global_m2_index("2015-01-01")
        btc_df = fetch_asset_price("BTC-USD", "10y")
        gold_df = fetch_asset_price("GC=F", "10y")
        silver_df = fetch_asset_price("SI=F", "10y")
    
    if m2_df is not None:
        # M2 vs Bitcoin (with 10-week lag)
        st.markdown("#### 💰 Global M2 Money Supply vs Assets")
        st.caption("M2 money supply expansion historically leads risk asset prices. Bitcoin is shown with a 10-week lag to visualize the leading relationship.")
        ...
        # M2 vs Gold & Silver side by side
        gc1, gc2 = st.columns(2)
        with gc1:
            fig_gold = create_m2_vs_asset_chart(m2_df, gold_df, "Gold", "#ffd700", lag_weeks=0)
            if fig_gold:
                st.plotly_chart(fig_gold, use_container_width=True, config=PLOTLY_CONFIG)
            else:
                st.warning("Could not load Gold data")
        with gc2:
            fig_silver = create_m2_vs_asset_chart(m2_df, silver_df, "Silver", "#c0c0c0", lag_weeks=0)
            if fig_silver:
                st.plotly_chart(fig_silver, use_container_width=True, config=PLOTLY_CONFIG)
            else:
                st.warning("Could not load Silver data")
    else:
        st.error("⚠️ Could not fetch Global M2 data from FRED. The API may be temporarily unavailable.")
    ...
    with st.spinner(f"Loading {country} indicators..."):
        ...
        for key, cfg in config.items():
            df = fetch_fred_series(cfg["fred"], "2010-01-01")
    ...
    st.caption("Source: FRED (Federal Reserve Economic Database) | Data updated monthly/quarterly depending on series")
